...
var (
	NoneExistDB = errors.New("none exist db")
)

// GetMysqlDB 获取默认数据库
func GetMysqlDB(c *MysqlConf) (*gorm.DB, error) {
	return GetDB(c)

}

func GetDB(dbInfo *MysqlConf) (*gorm.DB, error) {
	db, ok := dbMap.Load(dbInfo.DB)
	if !ok {
...
func InitMysqlDB(dbInfo *MysqlConf, SlowThreshold time.Duration, MaxIdleConns int, MaxOpenConns int) {
	var err error
	once.Do(func() {
		//默认数据库
		...
		dbMap.Store(dbInfo.DB, database)
	})
